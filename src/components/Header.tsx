...
//Note!
// for of function requires exact definition of reference(const checkList=everything)
// without return, it renders what I inteded to do.
//But I thought filter() seemed more sophisticated
const checkUnseen = (
  everything: { id: number; text: string; seen: boolean }[]
) => {
  const checkList = everything;
  const unSeenArray = checkList.filter((everyObj) => everyObj.seen === false);
  /* let unSeenArray = [];
  for (let everyObj of checkList) {
    if (everyObj.seen === false) {
      unSeenArray.push(everyObj);
    }
  } */
  return unSeenArray.length;
};
      ...
      <Flex full justifyBetween>
        ...
        <FlexItem>
          ...
          <HeaderIcon>
            ...
            <Number>
              <store.Consumer>
                {(store) => checkUnseen(store!.notification)}
              </store.Consumer>
            </Number>
          </HeaderIcon>
        </FlexItem>
